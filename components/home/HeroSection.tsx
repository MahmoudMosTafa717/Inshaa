...
export function HeroSection() {
  const cityBadges = [
    { name: "محافظة الفيوم", primary: true },
    { name: "مدينة 6 أكتوبر", primary: true },
    { name: "مدينة الشيخ زايد", primary: true },
    { name: "العاصمة الإدارية", primary: false },
    { name: "القاهرة الجديدة والتجمع", primary: false },
  ];
        ...
        <FadeInView direction="down" delay={0.1}>
          <div className="inline-flex items-center gap-2.5 bg-white/95 border border-brick-300 px-3.5 py-1.5 mb-6 text-xs font-mono text-slate-800 shadow-sm backdrop-blur-sm">
            ...
            <span className="font-semibold text-slate-900">
              مكتب إنشاء للهندسة // بإشراف مهندس استشاري / عماد الدين أمين (سجل نقابة المهندسين 1248/خ)
            </span>
          </div>
        </FadeInView>
          ...
          <div className="lg:col-span-7 space-y-6 text-right">
            ...
            <FadeInView direction="up" delay={0.3}>
              <p className="text-lg sm:text-xl text-slate-700 leading-relaxed max-w-2xl font-normal font-sans">
                نحول قطعة أرضك السكنية أو التجارية إلى صرح معماري وهندسي راقٍ. تصميمات متطابقة مع كود البناء المصري ECP، استخراج معتمد للتراخيص بمحافظة الفيوم وجهازي 6 أكتوبر والشيخ زايد والعاصمة، وإشراف موقعي صارم يضمن توفير حتى 18% من هدر الخرسانة والحديد.
              </p>
            </FadeInView>
            ...
            <FadeInView direction="up" delay={0.45}>
              <div className="pt-6 border-t border-paper-300 flex items-center gap-6 text-xs text-slate-600 font-mono">
                <div>
                  <span className="font-bold text-slate-900 text-base font-sans">200+</span> مشروع منجز
                </div>
              ...
              <div className="relative mx-auto max-w-md lg:max-w-none">
                ...
                <div className="relative border-2 border-slate-900 bg-white p-2 shadow-2xl cad-border">
                  <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-100">
                    <Image
                      src="https://images.unsplash.com/photo-1555636222-cae831e670b3?auto=format&fit=crop&w=1000&q=80"
                      alt="مشروع برج الصفوة بالفيوم - مكتب إنشاء للهندسة مهندس عماد الدين أمين"
                      fill
                      priority
                  ...
                  <div className="p-3 bg-paper-50 border-t border-slate-200 mt-2 flex items-center justify-between text-xs font-mono">
                    <span className="font-bold text-slate-900">برج ومول الصفوة // الفيوم</span>
                ...
                <div className="absolute -top-6 -left-6 bg-slate-900 text-white p-3 shadow-xl hidden sm:flex items-center gap-3 border-2 border-desert-500 z-20">
                  ...
                  <div>
                    <div className="text-xs font-mono text-slate-300">م. عماد الدين أمين</div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-end gap-2">
              {cityBadges.map((city) => (
                <span
                  key={city.name}
                  className={`text-xs px-3 py-1 font-medium transition-colors font-display ${
                    city.primary
                      ? "bg-brick-50 text-brick-900 border border-brick-300 font-bold"
                      : "bg-white border border-paper-300 text-slate-700"
                  }`}
                >
                  ✦ {city.name}
                </span>
